"use client";
import { ChevronLeft, ChevronRight, Home, RefreshCw, BookOpen } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        {!collapsed && (
          <span className="text-[#7C3AED] text-xl font-extrabold">BlogSummarizer</span>
        )}
        <Button
          variant="ghost"
          size="icon"
          onClick={() => setCollapsed(!collapsed)}
          className="text-[#7C3AED] hover:bg-transparent cursor-pointer"
        >
      ...
      <nav className="flex flex-col mt-6 space-y-2">
        <Button
          variant="ghost"
          className="justify-start text-[#1E293B] cursor-pointer"
          onClick={() => router.push("/")}
        >
      ...
      {!collapsed && (
        <div className="absolute bottom-4 left-4 text-xs text-gray-500">
          © {new Date().getFullYear()} BlogSummarizer
        </div>
      )}
    </aside>
  );
